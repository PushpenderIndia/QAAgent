/**
 * QAAgent Init Command
 *
 * Scaffolds a new BDD project with QAAgent integration into a .qa-agent directory
 */
...
const FRAMEWORKS = {
  'playwright-bdd': {
    ...
    dependencies: ['qa-agent', 'playwright-bdd', '@playwright/test', 'typescript', 'varlock'],
    devDependencies: ['@cucumber/cucumber'],
  },
  'cucumber': {
    name: 'Cucumber.js',
    description: 'Standalone Cucumber with Playwright',
    dependencies: ['qa-agent', '@cucumber/cucumber', '@playwright/test', 'typescript', 'varlock'],
    devDependencies: [],
  },
};
...
export async function init(cliFramework, options) {
  clack.intro(chalk.bgCyan.black(' 🤖 QAAgent Initialization '));

  const targetDir = resolve(process.cwd(), '.qa-agent');
  ...
  let featuresPath = await clack.text({
    message: 'Where should feature files live? (path relative to .qa-agent/)',
    initialValue: 'features',
    placeholder: 'features',
    hint: 'Use ../path/to/features to point outside .qa-agent/',
  });
  ...
  if (existsSync(targetDir)) {
    ...
    if (files.length > 0 && !files.every(f => f.startsWith('.'))) {
      const confirmOverride = await clack.confirm({
        message: `Directory .qa-agent is not empty. Overwrite and re-initialize?`,
        initialValue: false,
      });
  ...
  spinner.start(`Scaffolding ${FRAMEWORKS[framework].name} into .qa-agent...`);
  ...
  try {
    ...
    if (framework === 'playwright-bdd') {
      ...
      // Copy example feature files into .qa-agent/features/
    ...
    if (existsSync(stepsPath)) {
      ...
      if (agent === 'openCode') {
        ...
        content = content.replace(
          /import\s*\{([^}]*)\}\s*from\s*['"]qa-agent['"]/,
          (_, imports) => `import { ${imports.replace('claudeCode', 'openCode')} } from 'qa-agent'`
        );
        // Swap provider call
  ...
  const qaAgentVersion = 'qa-agent@github:pushpenderindia/QAAgent';
  ...
  const allDeps = [...frameworkConfig.dependencies, ...frameworkConfig.devDependencies, agentSdk].map(dep =>
    dep === 'qa-agent' ? qaAgentVersion : dep
  );
  ...
  try {
    ...
    console.error(chalk.red('You will need to run `npm install` manually inside .qa-agent'));
  }
  ...
  clack.outro(chalk.bold.green('🎉 .qa-agent scaffolding complete!'));
}
