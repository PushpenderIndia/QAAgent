    ...
    return {
        ...
        async run(prompt, {
            ...
            const { client, server } = await createOpencode({
                port,
                // Lets the Orchestrator cancel the spawned opencode server process on timeout.
                signal: abortController?.signal,
                config: {
                    mcp: {
                        [mcpServerName]: { type: 'remote', url: mcpUrl },
                    },
                },
            });
            ...
            // No documented way to cancel an in-flight prompt itself — best effort on
            // timeout is to stop consuming the SSE stream and tear down the local server.
            let aborted = false;
            const onAbort = () => {
                aborted = true;
                eventStream.return?.();
                    ...
                    if (type === 'message.part.updated' && properties.part) {
                        ...
                        if (part.type === 'tool') {
                            ...
                            if (part.state.status === 'error') {
                                ...
                                if (part.tool?.includes(assertionToolPattern)) {
                                    assertionFailed = true;
                                    assertionError = errorText;
                                }
                            }
                            // MCP isError:true is surfaced as ToolStateCompleted with error text in output
                            if (part.state.status === 'completed' && part.tool?.includes(assertionToolPattern)) {
